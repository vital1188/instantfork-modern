/*
  ...
  2. Functions
    - `generate_claim_code()` - Generates unique claim codes
    - `claim_deal(p_deal_id, p_user_id)` - Claims a deal for a user
    - `redeem_deal(p_claim_code, p_restaurant_id)` - Redeems a claimed deal

  3. Security
    - Enable RLS on `claimed_deals` table
    - Add policies for users to view/create their own claims
    - Add policies for restaurants to view/update claims for their deals

  4. Indexes
    - Performance indexes on user_id, deal_id, restaurant_id, claim_code, status
*/
